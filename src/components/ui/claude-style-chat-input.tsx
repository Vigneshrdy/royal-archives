...
import { Plus, ChevronDown, ArrowUp, X, FileText, Loader2, Check, Archive, Scale } from "lucide-react";
...
const FilePreviewCard: React.FC<FilePreviewCardProps> = ({ file, onRemove }) => {
    ...
    return (
        <div className="relative group flex-shrink-0 w-24 h-24 rounded-xl overflow-hidden border border-border bg-secondary animate-fade-in transition-all hover:border-muted-foreground">
            {isImage ? (
...
const PastedContentCard: React.FC<PastedContentCardProps> = ({ content, onRemove }) => {
    return (
        <div className="relative group flex-shrink-0 w-28 h-28 rounded-xl overflow-hidden border border-border bg-card animate-fade-in p-3 flex flex-col justify-between shadow-page">
            ...
            <button
                ...
                className="absolute top-2 right-2 p-[3px] bg-card border border-border rounded-full text-muted-foreground hover:text-foreground transition-colors shadow-sm opacity-0 group-hover:opacity-100"
            >
    ...
    return (
        <div className="relative" ref={dropdownRef}>
            <button
                ...
                className={`inline-flex items-center justify-center relative shrink-0 transition font-sans duration-300 ease-out h-8 rounded-lg px-3 min-w-[4rem] active:scale-[0.98] whitespace-nowrap text-xs pl-2.5 pr-2 gap-1 
                ${isOpen
            ...
            {isOpen && (
                <div className="absolute bottom-full right-0 mb-2 w-[260px] bg-card border border-border rounded-xl shadow-elevated overflow-hidden z-50 flex flex-col p-1.5 animate-scale-in origin-bottom-right">
                    {models.map(model => (
                        <button
                            key={model.id}
                            onClick={() => {
                            ...
                            className="w-full text-left px-3 py-2.5 rounded-lg flex items-start justify-between group transition-colors hover:bg-secondary"
                        >
                            <div className="flex flex-col gap-0.5">
                                <div className="flex items-center gap-2">
                                    ...
                                    {model.badge && (
                                        <span className="px-1.5 py-[1px] rounded-full text-[10px] font-medium border border-gold/30 text-gold bg-gold/10">
                                            {model.badge}
                                        </span>
                                    )}
                                </div>
                            ...
                            {selectedModel === model.id && (
                                <Icons.Check className="w-4 h-4 text-gold mt-1" />
                            )}
                        </button>
                    ))}
                    ...
                    <button className="w-full text-left px-3 py-2.5 rounded-lg flex items-center justify-between group transition-colors hover:bg-secondary text-foreground">
...
export const NyayaChatInput: React.FC<NyayaChatInputProps> = ({ onSendMessage }) => {
    const [message, setMessage] = useState("");
    const [files, setFiles] = useState<AttachedFile[]>([]);
    const [pastedContent, setPastedContent] = useState<{ id: string; content: string; timestamp: Date }[]>([]);
    const [isDragging, setIsDragging] = useState(false);
    const [selectedModel, setSelectedModel] = useState("nyaya-standard");
    ...
    const models = [
        { id: "nyaya-standard", name: "Nyaya Standard", description: "Best for everyday legal queries" },
        { id: "nyaya-research", name: "Nyaya Research", description: "Deep case law analysis", badge: "Pro" },
        { id: "nyaya-quick", name: "Nyaya Quick", description: "Fast answers for simple questions" }
    ];
    ...
    const handleFiles = useCallback((newFilesList: FileList | File[]) => {
        ...
        setMessage(prev => {
            ...
            if (newFiles.length === 1) {
                const f = newFiles[0];
                if (f.type.startsWith('image/')) return "Analyzing document...";
                return "Reviewing uploaded file...";
            }
            return `Reviewing ${newFiles.length} files...`;
        });
    ...
    const handlePaste = (e: React.ClipboardEvent) => {
        ...
        if (text.length > 300) {
            ...
            if (!message) {
                setMessage("Analyzing pasted legal text...");
            }
        }
    };
    ...
    return (
        <div
            ...
            onDrop={onDrop}
        >
            <div className="flex flex-col mx-2 md:mx-0 items-stretch transition-all duration-200 relative z-10 rounded-xl cursor-text border border-border shadow-book hover:shadow-elevated focus-within:shadow-elevated bg-card">
                    ...
                    <div className="relative mb-1">
                        <div className="max-h-96 w-full overflow-y-auto font-sans break-words transition-opacity duration-200 min-h-[2.5rem] pl-1">
                            <textarea
                                ...
                                className="w-full bg-transparent border-0 outline-none text-foreground text-[16px] placeholder:text-muted-foreground resize-none overflow-hidden py-0 leading-relaxed block font-normal"
                                rows={1}
                                autoFocus
                    ...
                    <div className="flex gap-2 w-full items-center">
                        <div className="relative flex-1 flex items-center shrink min-w-0 gap-1">
                            ...
                            <button
                                ...
                                className="inline-flex items-center justify-center relative shrink-0 transition-colors duration-200 h-8 w-8 rounded-lg active:scale-95 text-muted-foreground hover:text-foreground hover:bg-secondary"
                                type="button"
                        ...
                        <div className="flex flex-row items-center min-w-0 gap-1">
                            ...
                            <div>
                                <button
                                    ...
                                    disabled={!hasContent}
                                    className={`
                                        inline-flex items-center justify-center relative shrink-0 transition-colors h-8 w-8 rounded-lg active:scale-95
                                        ${hasContent
                                            ? 'bg-primary text-primary-foreground hover:bg-primary/90 shadow-emboss'
            ...
            {isDragging && (
                <div className="absolute inset-0 bg-secondary/90 border-2 border-dashed border-gold rounded-xl z-50 flex flex-col items-center justify-center backdrop-blur-sm pointer-events-none">
                    <Icons.Archive className="w-10 h-10 text-gold mb-2 animate-bounce" />
                    <p className="text-gold font-medium">Drop files to upload</p>
                </div>
            )}
